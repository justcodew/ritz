...
- **不做什么**：PDF 页面编辑（插入/删除页，plan_v1 Phase 5e）
- **现状**：0.2.0，单人项目，2026 开发，**继承 MuPDF 实场稳健性 + 在多个场景 2–10x 快于 PyMuPDF + Phase 5a–5d（search_for / get_toc / set_toc / 注释读写 / save / resolve_names）**
...
- [15-phase5e-page-editing.md](15-phase5e-page-editing.md) — 页面编辑实现（含 refcount 陷阱、跨 ctx insert_pdf 方案）
...
**62 个 pytest 必须全绿。这是合并任何 PR 的硬门。**
...
详见 [plan_v1 §5.5](../plan/01-plan-v1.md)"季度同步"。
...
当前 `metadata_cache`、`rect_cache`、`raw_doc` 都假设底层不可变。**Phase 5b/5c 已加注释写入和大纲写入**，save 后重新打开会得到新对象（缓存自然重置），但同一 doc 对象内连续写入+读取可能读到过期缓存。

**建议**：在任何写操作（`set_toc`、`add_*_annot`、`delete_annot`）后如需立即读取，重新 `open` 文档。
...
| patches/ 工作流 | ✅ |
| 文本搜索 `page.search_for()` | ✅ v0.2.0（Phase 5a） |
| 大纲读取 `doc.get_toc()` | ✅ v0.2.0（Phase 5a） |
| 注释读写 | ✅ Phase 5b（highlight/underline/strikeout/text + delete） |
| 大纲写入 `doc.set_toc()` | ✅ Phase 5c |
| PDF 写出 `doc.save()` | ✅ Phase 5c |
| 命名目标 `doc.resolve_names()` | ✅ Phase 5d |
| PDF 页面编辑（插入/删除页） | ✅ Phase 5e（new_page/delete_page/delete_pages/move_page/copy_page/insert_pdf）|
...
2. ~~**补文本搜索 `page.search_for()`**~~ ✅ 已在 v0.2.0 实现（Phase 5a）
3. ~~**补大纲 `doc.get_toc()`**~~ ✅ 已在 v0.2.0 实现（Phase 5a）
4. **图片直读 `pdf_load_object_stream`**（[docx/09 Tier 3 #11](09-acceleration-opportunities.md)，1 周）— 大赢面 3–20x
5. ~~**Phase 5b：注释读写**~~ ✅ 已实现（highlight/underline/strikeout/text + delete）
6. ~~**Phase 5c：大纲写入 `doc.set_toc()`**~~ ✅ 已实现
7. ~~**Phase 5d：`doc.resolve_names()` + `doc.save()`**~~ ✅ 已实现
8. ~~**PDF 页面编辑（插入/删除页）**~~ ✅ 已在 Phase 5e 实现（new_page/delete_page/delete_pages/move_page/copy_page/insert_pdf）
...
```
评估是否有值得合并的上游改动。详见 [plan_v1 §5.5](../plan/01-plan-v1.md)。
...
4. ~~**加 `page.search_for("query")`**~~ ✅ 已在 v0.2.0 实现
5. ~~**加 `doc.get_toc()`**~~ ✅ 已在 v0.2.0 实现
6. ~~**Phase 5b/5c/5d**~~ ✅ 注释读写 + set_toc + save + resolve_names 已实现
7. ~~**Phase 5e：页面编辑**~~ ✅ new_page/delete_page/delete_pages/move_page/copy_page/insert_pdf 已实现
